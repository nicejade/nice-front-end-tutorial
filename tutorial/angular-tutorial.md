## 基础篇

- [Angular.js 中文文档](https://angular.cn/)
- [Angular.js 中文社区](http://angularjs.cn)
- [Angular.js 英文资料](https://github.com/jmcunningham/AngularJS-Learning)
- [Angular.js 在线教程](http://each.sinaapp.com/angular)

## 进阶篇

- [Angular.js 数据绑定原理](https://github.com/Pasvaz/bindonce)

## 工具篇

- [mgechev/codelyzer](https://github.com/mgechev/codelyzer): <sub>Static analysis for Angular projects.</sub>

## 资源篇

- [Awesome Angular List](https://github.com/gdi2290/awesome-angular)
- [Angular UI 组件](https://github.com/angular-ui)
- [Angular Bootstrap 封装](http://angular-ui.github.io/bootstrap)
- [11 Angular Component Libraries You Should Know In 2018](https://blog.bitsrc.io/11-angular-component-libraries-you-should-know-in-2018-e9f9c9d544ff)

## 讯息篇

- [Whats New in Angular 6 ?](https://walkingtree.tech/whats-new-angular-6/)
